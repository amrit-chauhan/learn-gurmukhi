...
- `/words` — Practice Words hub (Common Words / Days of the Week / Numbers 0–100)
- `/word-study` — Practice-word flashcard session (Gurmukhi → romanization + translation + audio)
- `/stats` — Time-tracking statistics page

## Practice Words
- New "Practice Words" section for learning vocabulary (separate from the alphabet).
- Three sections: **Common Words** (~250 everyday words), **Days of the Week** (7), **Numbers 0–100** (spoken Punjabi number words).
- Data: `backend/data/word_data.py` (`PRACTICE_WORDS`), each `{id, gurmukhi, romanization, translation, category}`.
- Flashcard: front shows the Gurmukhi spelling; answer side shows how to say it (romanization) + English translation + a pronunciation button.
...
- API: `GET /api/words` (list) and `GET /api/words/tts/{word_id}` (mp3).
- Reuses the letter study engine (`useStudySession` with an explicit card pool) and per-item progress tracking.
